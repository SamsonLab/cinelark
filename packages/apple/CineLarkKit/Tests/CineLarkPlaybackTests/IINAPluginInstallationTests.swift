...
struct IINAPluginInstallationTests {
    ...
    func outdatedPluginRequiresUpdate() throws {
        ...
        #expect(IINAPluginInstallation(directoryURL: directory).requiresVersion("0.1.7"))
    }

    @Test("current and newer plugins remain installed")
    func currentAndNewerPluginsRemainInstalled() throws {
        let current = try makePlugin(version: "0.1.7")
        ...
        #expect(!IINAPluginInstallation(directoryURL: current).requiresVersion("0.1.7"))
        #expect(!IINAPluginInstallation(directoryURL: newer).requiresVersion("0.1.7"))
    }
